...
The microservice you will be working is calld `system`. The `system` service returns the JVM system properties of the running container. This guide demonstrates how to build this microservice with `Paketo Buildpacks` and run them with your preferred container manager.
...
Explore all of the flags, variables and options available for the Paketo Liberty buildpack on the

https://github.com/paketo-buildpacks/liberty#configuration[Paketo Liberty page^]. 

== Running your microservices in containers

Now that your `system` image is built, you will now run it in a container. Since the final images that are generated from `Pack CLI` are Open Containers Initiative (OCI) images, they are compatible with whichever container manager you have configured with `Pack CLI`.
...
You can test your `system` service manually by hitting the endpoints or with automated tests that check your running container.
