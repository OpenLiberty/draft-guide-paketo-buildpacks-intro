...
== Building the images with Paketo buildpacks

Before attempting to build your images, ensure that you have installed the https://buildpacks.io/docs/tools/pack/[Pack CLI] as it is needed to run the build processes with Paketo.

Paketo buildpacks are an open source project which provides users with production-ready buildpacks for the most popular languages and framworks, such as the Open Liberty runtime.

A buildpack is a set of executables that is able to inspect and transform your application source code into container images that conform to Open Container Initiative (OCI) image standards.
...
* Generate Container images without a Dockerfile.
* Advanced Caching for faster rebuild times of your application.
* Software Bill-of-Materials (SBOM) which provides information on the contents of your final image.
* Optimized image size to provide the minimal size for final application image.
* Reproduces the same application image digest by re-running the build.
* Auto-Detection of application dependents directly from application source.
* Image reabasing by repatching the OS layer of your image.
...
* Set of buildpacks that apply your applications dependencies.
* A stack, which provides the OS layer for your appliation image.
* CNB lifecycle which analyzes, detects and assembles your final application image.
