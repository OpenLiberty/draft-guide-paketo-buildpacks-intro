...
With `Paketo Buildpacks`, you are able to compile and create your microservice image with a single command and no other provided information. When you use the `Pack CLI` with the `Paketo Buildpacks` the `pack build` command will inspect, analyze and constuct your Open Liberty microservice into an Open Containers Initiative (OCI) image.
