...
To construct your `system` and `inventory` services image, begin with the following `pack build` commands and the applicable flags and variables to customize your Open Liberty image:
...
```
Then, run the following command to build the `inventory` service:
[role='command']
```
