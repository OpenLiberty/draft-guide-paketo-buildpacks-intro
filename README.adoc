...
Learn how to build your microservices with Paketo Buildpacks and Open Liberty and run them in a container manager.
...
With `Paketo Buildpacks`, you are able to utilize `Pack CLI` to compile and create your application image with a single command and no other provided information.
When you use the `Paketo` builder and buildpacks with the `pack build` command, then `Pack CLI` will inspect, analyze and construct your microservices into Open Containers Initiative (OCI) images.

Once your microservice image is built, then you are able to deploy them in different environments in a lightweight and portable manner by using containers and your preferred container manager such as `Podman` or `Docker`.

The microservice that you'll be working with is called `system`. The `system` miroservice returns the JVM system properties of the running container. This guide demonstrates how microservices can be built with `Paketo buildpacks` and run them with your preferred container manager.
...
To package your microservice, run the `mvn package` goal to build the application `.war` files from the start directory so that the `.war` file is in the `system/target` directory.
...
The default builder is the base builder used by all other commands that are run with the `Pack CLI`, in this instance we set the default builder to the `Paketo Buildpacks` builder to use the features it offers.
...
After running your `pack build` command, the following output verifies that your image can now be ran in a container:
[source, role='no_copy']
----
Successfully built image system:1.0-SNAPSHOT
----

Review the tables below to better understand the anatomy of the `pack build` command you used to construct your images.
...
=== Image directory structure for Paketo Buildpack images

Add information abot the directory strcture being mutated from typcial structure.

== Running your microservices in containers

Now that your `system` image is built, you will now run your microservice in a container. Since the final images that are generated from `Pack CLI` are Open Containers Initiative (OCI) images, they are compatible with the container manager you have configured with `Pack CLI`.
...
[.tab_content.podman_section]
--
If a problem occurs and your container exits prematurely, the container won't appear in the container list that the `podman ps` command displays. Instead, your container appear with an `Exited` status when they run the `podman ps -a` command. Run the `podman logs system` command to view the container logs for any potential problems. Run the `podman stats system` command to display a live stream of usage statistics for your container. When you find the cause of the issues, remove the faulty container with the `podman stop system` command. Rebuild your image, and start the container again.
--
...
[.tab_content.docker_section]
--
You can test your microservices manually by hitting the endpoints or with automated tests that check your running Docker container.
--

[.tab_content.podman_section]
--
You can test your microservices manually by hitting the endpoint or with automated test that check your running `Podman` container.
--
...
Begin with getting the IP address of your container that is running your `system` service with the following command:
...
[.tab_content.docker_section]
--
Next, run the Maven `package` goal to compile the test classes. Run the Maven `failsafe` goal to test the service that is running in the `Docker` container by replacing the `[system-ip-address]` with the IP address that you determined previously.
--

[.tab_content.podman_section]
--
Next, run the Maven `package` goal to compile the test classes. Run the Maven `failsafe` goal to test the services that are running in the `Podman` containers by replacing the `[system-ip-address]` with the IP address that you determined previously.
--
