...
The benefits of using `Paketo buildpacks` to build your application compared to other build tools such as `docker build` and `podman build`, include:
...
Now that your `system` image is built, you will run your microservice in a container. In this guide, you will use `Docker` as the container manager for deploying your image, but the process should be neary the same for all other container managers.
