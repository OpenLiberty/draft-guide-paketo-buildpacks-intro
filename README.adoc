...
--
Run the following command if the container mangager you are using is `Docker` to build the `system` service:
[role='command']
```
pack build --path ./system \
--env BP_JAVA_APP_SERVER=liberty \
...
Otherwise, if `Podman` is your current container manger, run the following command
[role='command']
```
pack build --path ./system \
--env BP_JAVA_APP_SERVER=liberty \
