...
You'll learn how to build your microservices into Open Containers Initiative (OCI) images with `Paketo Buildpacks`. By specifying the buildpacks and builder to be utilized in the `pack build` command, the `Pack CLI` will construct your microservices into container images. Once your microservice image is built, your are then able to deploy them in different environments in a lightweight and portable manner by using containers with preferred container manager such as `Podman` or `Docker`.
