...
You will learn to build your microservice images with `Paketo Buildpacks`. When you use the `Pack CLI` with the `Paketo Buildpacks`, the `pack build` command will inspect, analyze and constuct your Open Liberty microservice into an Open Containers Initiative (OCI) image.
...
The default builder is the builder used by all other commands that are run with the `Pack CLI`, in this instance, we set the default builder to the `Paketo Buildpacks` builder to use the features it offers.
...
You can test your microservice manually by hitting the endpoints or with automated tests that check your running container.
...
Next, run the Maven `package` goal to compile the test classes. Run the Maven `failsafe` goal to test the service that is running in the container by replacing the `[system-ip-address]` with the IP address that you determined previously.
...
You have just built a microservice image with `Paketo Buildpacks` and run it in a container. 
