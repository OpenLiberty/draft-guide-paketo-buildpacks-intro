...
`Paketo` buildpacks are an open source project which provides users with production-ready buildpacks for the most popular languages and framworks, such as the `Liberty` runtime. A `buildpack` is a set of executables that is able to inspect and transform your application source code into container images that conform to Open Container Initiative (OCI) image standards.
...
* Auto-detects your app's dependencies directly from the source code.
* Patch OS-Level vulnerabilites in your images automatically.
...
Explore all of the flags, variables and options available for the Liberty buildpack on the https://github.com/paketo-buildpacks/liberty[Paketo Liberty page^]. 
