...
The benefits of using `Paketo buildpacks` with the `Pack CLI` to build your application compared to toher traditional build tools, include:
...
Now, to begin constructing your `system` service image, run the following `pack build` command with the applicabke flags envir varibales to customize your Open Liberty image:
include::{common-includes}/os-tabs.adoc[]

[.tab_content.windows_section]
--
Currently, `Pack CLI` does not officially support the building images for `Podman` on Windows, so for the time being, you are limited to using `Docker` exclusively.
...
| --docker-host           | Address to docker daemon that will be exposed to build container. The `inherit` value may be used when the address is set to DOCKER_HOST terminal environment variable.
| --trust-builder         | Trust the provided builder, all lifecycle phases will be run in single container.
|===
