...
You'll learn how to build your microservices into OCI images with `Paketo` buildpacks and run them with Docker. You'll specify the buildpacks and builder to be utilized in the `pack build` command and run your images in Docker containers by using `docker run` command.
...
Before attempting to build your images, ensure that you have installed https://buildpacks.io/docs/tools/pack/[Pack CLI]. The Pack CLI is a command line tool that facilitates the use of buildpacks and provides functionality to build applications with buildpacks and rebase application images created using buildpacks.

`Paketo` buildpacks are an open source project which provides users with production-ready buildpacks for the most popular languages and framworks, such as the Open Liberty runtime. A `buildpack` is a set of executables that inspects and transforms your application source code into Open Container Initiative (OCI) images that can be ran in nearly any container manager.
...
The benefits of using `Paketo` buildpacks compared to other build systems such as `docker build` inlclude the following:
...
A `builder` in a buildpack system, is an image comprised of three parts:
...
Next, construct your images with the following `pack build` command and the applicable flags and variables to customize your Open Liberty image:
[role='command']
```
...
As well, learn more about Docker on the https://www.docker.com/what-docker[official Docker page^].
