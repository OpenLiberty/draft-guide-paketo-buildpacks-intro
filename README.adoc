...
You will learn to build a microservice image with `Paketo Buildpacks`. When you use the `Pack CLI` with the `Paketo Buildpacks`, the `pack build` coammnd will inspect, analyze and construct your Open Liberty microservice into an Open Containers Initiative (OCI) image.
...
[role='command']
----
pack build --path ./system --env BP_JAVA_APP_SERVER=liberty --env BP_MAVEN_BUILT_ARTIFACT="target/*.[ejw]ar src/main/liberty/config/*" --env BP_LIBERTY_PROFILE=jakartaee9 --buildpack paketo-buildpacks/eclipse-openj9 --buildpack paketo-buildpacks/java --creation-time now system:1.0-SNAPSHOT
----
--
...
[role='command']
----
pack build --path ./system --env BP_JAVA_APP_SERVER=liberty --env BP_MAVEN_BUILT_ARTIFACT="target/*.[ejw]ar src/main/liberty/config/*" --env BP_LIBERTY_PROFILE=jakartaee9 --buildpack paketo-buildpacks/eclipse-openj9 --buildpack paketo-buildpacks/java --docker-host=inherit --trust-builder=true --creation-time now system:1.0-SNAPSHOT
----
...
When building your Open Liberty image with `Podman` or `Docker` the structure of te image is similar to the following:

* `/opt/ol/wlp` contains the Liberty runtime.
* `/config` contains the server configuration.

This strucutre is something that is typcially expected if you are familiar with building Open Liberty images. Although, when using `Paketo Buildpacks`, the image takes a much different structure. The `Paketo Buildpacks` strucutre is as follows:

* `/layers/paketo-buildpacks_liberty/open-liberty-runtime-kernel` contains the Liberty runtime.
* `/layers/paketo-buildpacks_liberty/open-liberty-runtime-kernel/usr/servers/defaultServe` contains the server configuration.

Notice how all of the files have been placed under the `/layers` director. This is because `Buildpacks` do not run as `root`, so the `cnb` user/group is created and uses the `/layers` directory as the location that `Buildpacks` are able to write to.

To find out more information on the structure of your image, refer to the https://github.com/buildpacks/spec/blob/main/buildpack.md[Buildpack Interface Specification page^]. Otherwise, continue with the guide to run your newly built image.
