...
:page-seo-title: Building and containerizing microservices wit Paketo Buildpacks 
...
You'll learn how to build your microservices into OCI images with `Paketo` buildpacks and run them with your preferred container manager, such as `Docker` or `Podman`. You'll specify the buildpacks and builder to be utilized in the `pack build` command and run your images in containers.
...
Before starting the guide, ensure that you have installed the `Pack CLI` and a container manager such as `Docker` or `Podman`. For installation, refer to the https://buildpacks.io/docs/tools/pack/[offical Pack documentation^] and the https://docs.docker.com/get-docker/[official Docker documentation^].

// Potentially remove this now I'm thinking?
...
To package your microservice, run the maven package goal to build he application `.war` files from the start directory so that the `.war` file is in the `system/target` directory.
...
* Generating your application images without a Dockerfile or Containerfile.
...
To construct your `system` service image, begin with the following `pack build` commands and the applicable flags and variables to customize your Open Liberty image:
...
To access the application, go to `http://localhost:9080/system/properties` and there will be JSON container information about the container that is currently running the `system` service.
