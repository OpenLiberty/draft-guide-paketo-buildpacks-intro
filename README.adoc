...
`Paketo` offers many benefits compared to more traditonal ways of generating images from methods such as using `docker build`, which include:

The benefits of using `Paketo` over other build methods such as `docker build`, include:

* Generating container images without a Dockerfile.
...
* Optimizes image size for minimal final application image.
* Auto-detectsion of your app's dependencies directly from it's source code.
* Patches OS-Level vulnerabilites in your images automatically.
...
The `paketo-buildpacks` is the `builder:base` version which is the most basic version of builder to interact with your source code. There is an option, `builder:full` which contains more builders if the `builder:base` is insufficient.

A `builder` is an image comprised of three parts:
