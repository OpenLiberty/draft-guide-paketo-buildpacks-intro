...
You'll learn how to build your microservices into Open Containers Initiative (OCI) images with `Paketo Buildpacks` and run them with your preferred container manager, such as `Podman` or `Docker`. You'll specify the buildpacks and builder to be utilized in the `pack build` command to construct your microservice image. Once your microservice images are built your are then able to deploy them in different environments in a lightweight and portable manner by using containers and youir


You'll learn how to build your microservices into Open Containers Initiative (OCI) images with `Paketo Buildpacks`. By specifying the buildpacks and builder to be utilized in the `pack build` command to construct your microservice image. Once your microservice image is built your are then able to deploy them in different environments in a lightweight and portable manner by using containers with preferred container manager such as `Podman` or `Docker`.
