...
When building your application images for your microservices, there is often a lot of overhead that is required to construct your images, such as creating a Containerfile or Dockerfile or ensuring all dependencies are installed. 

With `Paketo Buildpacks`, you are able to utilize the `Pack CLI` to compile and create your application image with a single command and no other provided information.
When you use the `Paketo` builder and builderpacks with the `pack build` command, the `Pack CLI` will then inspect, analyze and construct your microservices into Open Containers Initiative (OCI) images.

Once your microservice image is built, your are then able to deploy them in different environments in a lightweight and portable manner by using containers with preferred container manager such as `Podman` or `Docker`.
...
Review the tables below to understand better the anatomy of the `pack build` command you used to construct your images.
...
Review the following table that describes the flags in the above commands:
