...
You will learn to build your microservice images using the `Paketo Buildpacks`. When you use the `Pack CLI` with the `Paketo Buildpacks`, the `pack build` command will inspect, analyze and constuct your Open Liberty microservice into an Open Containers Initiative (OCI) image.
...
[.tab_content.podman_section]
--
Next, run the Maven `package` goal to compile the test classes. Run the Maven `failsafe` goal to test the service that are running in the `Podman` containers by replacing the `[system-ip-address]` with the IP address that you determined previously.
--
