...
Before attempting to build your images, ensure that you have installed the https://buildpacks.io/docs/tools/pack/[Pack CLI] as it is needed to run the build processes for `Paketo`.
...
Before building your images, set your default builder to the `paketo-buildpacks` builder with the following command:
[role='command']
```
...
The default builder you for yourself is the `builder:base`. This builder is the most bare-bones of the `Paketo buildpacks` providing only what is necessary to alayze, detect and build your application.
