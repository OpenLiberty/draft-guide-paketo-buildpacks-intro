...
pack build --path ./inventory --env BP_JAVA_APP_SERVER=liberty --env BP_MAVEN_BUILT_ARTIFACT="target/*.[ejw]ar src/main/liberty/config/*" --env BP_LIBERTY_PROFILE=jakartaee9 --buildpack paketo-buildpacks/eclipse-openj9 --buildpack paketo-buildpacks/java inventory:1.0-SNAPSHOT
```
...
| --env BP_JAVA_APP_SERVER      | The application server to use, in the case it's `Liberty`
| --env BP_MAVEN_BUILT_ARTIFACT | Include the `server.xml` information from Maven into the `pack build` process.
| --env BP_LIBERTY_PROFILE      | The Liberty profile to use. Defaults to `full`.
|===
